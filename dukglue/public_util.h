...
// std::map to DukValue   {key0:val0, key1:val1, key2:val2}
template<typename MAP>
DukValue dukglue_map_to_obj(duk_context* ctx, const MAP& mp)
{
	int i = 0;
	for (MAP::const_iterator cit = mp.cbegin(); cit != mp.cend(); ++cit)
	{
		std::string sIdx = std::to_string(i).c_str();
		dukglue_register_global(ctx, cit->first, ("__map_key_" + sIdx).c_str());
		dukglue_register_global(ctx, cit->second, ("__map_val_" + sIdx).c_str());
		++i;
	}

	dukglue_peval<DukValue>(ctx, "var __map__ = {};");

	char strTemp[256];
	for (int j = 0; j < i; ++j)
	{
		sprintf(strTemp, "__map__[__map_key_%d] = __map_val_%d;", j, j);
		dukglue_peval<DukValue>(ctx, strTemp);
	}

	return dukglue_peval<DukValue>(ctx, "__map__");
}

// std::map to array   [{key:key0, val:val0}, {key:key1, val:val1}, {key:key2, val:val2}]
template<typename MAP>
DukValue dukglue_map_to_array(duk_context* ctx, const MAP& mp)
{
	int i = 0;
	for (MAP::const_iterator cit = mp.cbegin(); cit != mp.cend(); ++cit)
	{
	...
	for (int j = 0; j < i; ++j)
	{
		sprintf(strTemp, "__map__.push({key:__map_key_%d, val:__map_val_%d});", j, j);
		dukglue_peval<DukValue>(ctx, strTemp);
	}
...
// std::set to array   [key0, key1, key2]
template<typename SET>
DukValue dukglue_set_to_array(duk_context* ctx, const SET& st)
{
	int i = 0;
	for (SET::const_iterator cit = st.cbegin(); cit != st.cend(); ++cit)
	{
		std::string sIdx = std::to_string(i);
		dukglue_register_global(ctx, cit->first, ("__set_key_" + sIdx).c_str());
		++i;
	}

	dukglue_peval<DukValue>(ctx, "var __set__ = [];");

	char strTemp[256];
	for (int j = 0; j < i; ++j)
	{
		sprintf(strTemp, "__set__.push(__set_key_%d);", j);
		dukglue_peval<DukValue>(ctx, strTemp);
	}

	return dukglue_peval<DukValue>(ctx, "__set__");
}