...
namespace dukglue
{
	namespace detail
	{
		extern std::vector<duk_context*> stackThis;
		static duk_context* dukglue_this()
		{
			if (stackThis.size() == 0)
				return NULL;
			return stackThis.back();
		}
...
#define JS_THIS (dukglue::detail::dukglue_this())